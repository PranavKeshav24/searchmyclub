...
import { databases } from '../appwrite.config'; // Adjust the path if needed
  ...
  const subscribeToNewsletter = async () => {
    try {
      const response = await databases.createDocument(
        process.env.REACT_APP_DATABASE_ID, // Correct Database ID for 'myclub'
        '6720dce9001835d56187', // Collection ID for 'NewsLetter'
        'unique()', // Unique document ID, auto-generated
        { email } // Document data (fields and values)
      );
          ...
          <div className="sm:col-span-2">
            ...
            <div className="flex flex-col mx-auto mt-6 space-y-3 md:space-y-0 md:flex-row">
              <input
                ...
                className="px-4 py-2 text-gray-700 bg-white border rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-opacity-40 focus:ring-indigo-300"
              ...
              >
                <Link className="rounded-2xl flex px-6 py-2.5 text-white bg-black text-center font-medium cursor-pointer">
                  Subscribe {'\u003E'}
                </Link>
              </motion.div>
            </div>
